...
def getCountryInfo(file_name):
    data = {}
    with open(file_name) as f:
        data = json.load(f)
    countryInfo = {}
    for element in data['confirmed']['locations']:
        ...
        for date in element['history']:
            ...
            dates.append(date)
            confirmeds.append(count)
        if countryName not in countryInfo:
            countryInfo[countryName] = {}
            countryInfo[countryName]['dates'] = dates
            countryInfo[countryName]['confirmeds'] = confirmeds
        else:
            for i in range(len(countryInfo[countryName]['confirmeds'])):
                countryInfo[countryName]['confirmeds'][i] += confirmeds[i]

    for element in countryInfo.values():
        index = 0
        while index < len(element['dates']) and element['confirmeds'][index] == 0:
            index += 1
        element['dates'] = element['dates'][index:]
        element['confirmeds'] = element['confirmeds'][index:]
...
def updatehistories(currentData, lastDate):
    g = os.walk('./prediction/')
    for path, dir_list, file_list in g:
        for file_name in file_list:
            filepath = os.path.join(path, file_name)
            if 'confirmed' not in filepath:
                continue
                ...
                for countryName in hdata:
                    ...
                    for i in range(len(hdata['dates'])):
                        ...
                        if element['real'][i] == 0 and hdata['dates'][i] in currentData[countryName]['dates'] and datetime.strptime(hdata['dates'][i], "%m/%d/%y").date() <= lastDate:
                            element['real'][i] = currentData[countryName]['confirmeds'][currentData[countryName]['dates'].index(hdata['dates'][i])]
...
def learnParameters(country):
    ...
    for index in range(len(numbers)):
        ...
        if index == 0:
            ...
            simulatedConfirmed = simulatedResults[:,9]
            for i in range(len(simulatedConfirmed) - window):
                loss = L1_loss(numbers[:window], simulatedConfirmed[i:i+window])
                if loss < maxLoss:
                    maxLoss = loss
            ...
            for C in np.arange(10,10001,500):
                ...
                confirmedP = outputs[1:, 9]

                confirmedLoss = L1_loss(confirmedP, numberWindow)
                confirmedLoss /= window
                if confirmedLoss < maxLoss:
                    maxLoss = confirmedLoss
                    C_selected = C
...
def getPredictions(yList, cList):
    ...
    simulatedConfirmed = simulatedResults[:, 9][1:]
...
if __name__ == '__main__':
    ...
    for timediff in [0]:
        ...
        data = getCountryInfo(input_file)
        for element in data:
            ...
            data[element]['confirmeds'] = data[element]['confirmeds'][:index + 1]
        ...
        # history = {}
        # with open(output_path + datetime.strftime(currentdate - timedelta(days=1), '%Y-%m-%d') + '-confirmed.json', 'r') as f:
        #     history = json.load(f)

        outputfile = output_path + date + '-' + type + ".json"
        ...
        for countryName in data:
            try:
                ...
                for i in range(0,5):
                    ...
                    real.append(countryData['confirmeds'][i - 5])
                ...
                # real[-1] = countryData['confirmeds'][-1]
